...
/**
 * Read lines from a file and parse it as BSON document/s
 *
 ...
 *
 */
public class BsonDataSet extends ConfigTestElement implements TestBean, LoopIterationListener, NoConfigMerge {
    ...
    private static final String EOFVALUE = // value to return at EOF
            JMeterUtils.getPropDefault("bsondataset.eofstring", "<EOF>"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void iterationStart(LoopIterationEvent iterEvent) {
        ...
        if (var==null){
            ...
            int modeInt = BsonDataSetBeanInfo.getShareModeAsInt(mode);
            switch(modeInt){
                case BsonDataSetBeanInfo.SHARE_ALL:
                    alias = _fileName;
                    break;
                case BsonDataSetBeanInfo.SHARE_GROUP:
                    alias = _fileName+"@"+System.identityHashCode(context.getThreadGroup());
                    break;
                case BsonDataSetBeanInfo.SHARE_THREAD:
            ...
            var = getVariableName();
            server.reserveFile(_fileName, "UTF-8", alias);
        }
        ...
        try {
            if (batchSize==0 || batchSize==1){
                line = server.readLine(alias, recycle);
                Document parse = Document.parse(line);
                threadVars.putObject(var, parse);
            }else{
                ...
                threadVars.putObject(var, docs);
            }
        } catch (IOException e) { // treat the same as EOF
            log.error(e.toString());
        }
        if (line==null ||  line.length() == 0) {// i.e. EOF
            if (getStopThread()) {
                throw new JMeterStopThreadException("End of file:"+ getFilename()+" detected for Bson DataSet:"
